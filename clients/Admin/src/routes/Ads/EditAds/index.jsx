...
class Registration extends Component {
  ...
  componentDidMount = async () => {
    this.setState({ loading: true }, async () => {
      ...
      const result = await axios(`/api/v1/ads/${id}`);
      const { data } = result;
      ...
      this.setState({ loading: false }, async () => {
        const fileList = [];
        await axios.get(`/api/v1/getFile/${media}`).then((result) => {
          fileList.push({
            uid: '-1',
            name: 'image.png',
            status: 'done',
            url: `/api/v1/getFile/${media}`,
          });
        }).catch((error) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (fileList.length !== 0) {
          ...
          axios.post('/api/v1/ads/update', { values, params: { id } }).then((result) => {
