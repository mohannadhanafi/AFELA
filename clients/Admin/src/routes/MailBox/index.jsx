...
class Mail extends PureComponent {
    ...
    Promise.all(
      this.state.allMail.map(async (mail) => {
        if (mail.selected && mail.folder === this.state.selectedFolder) {
          ...
          await axios.post('/api/v1/edit/detele', { deleted, id });
          if (mail.folder === 1) {
            return {
  ...
  componentDidMount() {
    axios.get('/api/v1/getMails').then((result) => {
      const { data } = result;
      const folderMails = data.filter(mail => mail.folder === 0);
  ...
  onStartSelect(data) {
    ...
    axios.post('/api/v1/edit/starred', { starred, id }).then((result) => {
      ...
      this.setState({
        ...
        NotificationManager.success(alertMessage, null, 1500);
      });
    });
  }
  ...
  onImportantSelect(data) {
    ...
    axios.post('/api/v1/edit/important', { important, id }).then((result) => {
