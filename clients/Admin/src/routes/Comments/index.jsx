...
const expandedRowRender = record => <p>{record.title}</p>;
...
class Dynamic extends React.Component {
  ...
  delete = (id) => {
    axios.post('/api/v1/comment/delete', { data: { id } }).then((res) => {
  ...
  update = (id) => {
    axios.post('/api/v1/comment/update', { data: { id } }).then((res) => {
      const { data: { message }, statusText } = res;
      if (res.status === 200) {
        const { data } = this.state;
  ...
  getData = () => {
    this.setState({ loading: true }, async () => {
      axios.get('/api/v1/comments/getAll').then((result) => {
        const { data } = result;
        const columns = [
          {
            title: 'Email',
        ...
        data.map((element) => {
          ...
          element.post = (
            <Link to={element.post ? `/admin/posts/${element.post.id}` : 'posts'}>
