...
class Dynamic extends React.Component {
  ...
  delete = (id) => {
    axios.delete(`/api/v1/statistics/${id}`).then((res) => {
      const {
        data: { message },
        statusText,
      } = res;
    ...
    componentWillMount = () => {
      axios.get('/api/v1/statistics').then((result) => {
        const { data } = result;
        const columns = [
          {
            title: 'title',
        ...
        data.map((element) => {
          element.action = (
            <span>
              <Link to={`/admin/statistics/${element.id}`} className="icon icon-feedback" />
              <Divider type="vertical" />
              <Popconfirm
                title="Are you sure delete this statistic?"
          ...
          element.title = (
            <Link to={`/admin/statistics/${element.id}`}>{element.title}</Link>
          );
          return element;
        });
        this.setState(() => ({ data }));
