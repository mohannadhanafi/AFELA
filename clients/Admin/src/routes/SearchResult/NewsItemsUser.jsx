...
function CryptoNewsItem({ data }) {
  const {
    name, pic, bio, createdAt, id,
  } = data;
  return (
    <div className="gx-news-item">
      <div className="gx-news-thumb"><img className="gx-width-115 gx-rounded-lg" src={`/api/v1/getFile/${pic}`} alt="..." /></div>
      <div className="gx-news-content">
        <h4 className="gx-mt-0">{name}</h4>
        <p className="gx-mb-2">{bio}</p>
